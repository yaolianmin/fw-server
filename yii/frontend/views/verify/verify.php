...
for($i=0;$i<4;$i++){
	...
	imagestring($img,5, 13*$i+2, rand(1,9), $tmp, $color);
...
// }
     ...
     */
        ...
        // 曲线前部分
        $A = mt_rand(1, 12);                  // 振幅
        $b = mt_rand(-10, 10);   // Y轴方向偏移量
        $f = mt_rand(-10, 10);   // X轴方向偏移量
        $T = mt_rand(24, 25);  // 周期
        $w = (2 * M_PI) / $T;
        ...
        $color = imagecolorallocate($img, 20, 132, 232);
        ...
        $color = imagecolorallocate($img, 20, 132, 231);
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if ($w != 0) {
                $py = $A * sin($w * $px + $f) + $b + 12;  // y = Asin(ωx+φ) + b
                $i = 5;
...
setcookie("verify",$verify,time()+900);
